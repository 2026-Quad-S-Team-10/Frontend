      ...
      <div className="bottom-sheet-content" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="calendar-grid">
          ...
          {dates.map((date, index) => {
            ...
            const hasStar = learnedDates.includes(toDateKey(date));
            const isToday =
              date === todayDate &&
        ...
        <div className="summary-cards">
          <div className="summary-card">
            ...
            <span className="summary-value">
              {calendarData?.currentStreak ?? 0}일
            </span>
          </div>
          ...
          <div className="summary-card">
            ...
            <span className="summary-value">
              {calendarData?.maxMonthlyStreak ?? 0}일
            </span>
          </div>
        </div>
      </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            >
              ...
              <span style={{ fontWeight: 700 }}>
                {gradeData.progress ??
                  `${gradeData.correctQuizCount ?? 0} / ${gradeData.requiredCorrectCount ?? 10}`}
              </span>
            </div>
          </div>
        </div>
...
const HomePage = () => {
  ...
  useEffect(() => {
    Promise.allSettled([getStreak(), getGrade(), getCharacterInfo()])
      .then(([streakRes, gradeRes, charRes]) => {
        ...
        if (charRes.status === 'fulfilled') {
          setCharacterData(charRes.value);
        }
      })
  ...
  const currentWord =
    characterData?.speechBubbleMessage ??
    characterData?.speechBubbleText ??
  ...
  const characterImageUrl =
    ...
    characterData?.equippedFaceAssetKey ??
    null;
  ...
  const weekDays = DAYS.map((day, index) => ({
    label: day,
    isToday: index === todayDayIdx,
    isFilled: index <= todayDayIdx && todayDayIdx - index < streakDays,
  }));
      ...
      <div className="home-content">
        <div className="grade-bar" onClick={() => setIsSettingsOpen(true)}>
          <div className="grade-level">
            ...
            <div className="grade-color-circle" />
          </div>
        ...
        <div className="streak-card" onClick={() => setIsCalendarOpen(true)}>
          ...
          <div className="streak-days">
            {weekDays.map((day) =>
              day.isFilled ? (
                <span key={day.label} className="streak-day-icon">
                  ⭐
                </span>
              ) : (
                <span
                  key={day.label}
                  className={`streak-day-item${day.isToday ? ' today' : ''}`}
                >
                  {day.label}
                </span>
              ),
